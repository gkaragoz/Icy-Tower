...
public class TextLocalisedSearchWindow : EditorWindow
{
    ...
    {
        TextLocalisedSearchWindow window = new TextLocalisedSearchWindow();
    ...
    void GetSearchResults()
    {
        ...
        foreach (KeyValuePair<string,string> element in dictionary)
        {
            if (element.Key.ToLower().Contains(value.ToLower())||element.Value.ToLower().Contains(value.ToLower()))
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Remove Key "+element.Key+"?"," This will remove elemet from localisation.Are you Sure?","Do it"))
                    {
